...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint, debug_from,
    ...
    # Compatibility shim: loss weights present in the 2024 fudan trainer but
    # absent from OMG4's arguments module (all default to disabled).
    for _k in ('lambda_opa_mask', 'lambda_rigid', 'lambda_motion'):
        if not hasattr(opt, _k):
            setattr(opt, _k, 0.0)
    ...
    while iteration < opt.iterations + 1:
        for batch_data in training_dataloader:
            ...
            for batch_idx in range(batch_size):
                ...
                # Anisotropy penalty. With few cameras and silhouette-only masks
                # depth along the view ray is under-constrained, so Gaussians
                # stretch into needles aligned with the training views: nearly
                # invisible end-on from those cameras, but long streaks from any
                ...
                if opt.lambda_aniso > 0:
                    ...
                    # log-space: the raw ratio is heavy-tailed (heidi median 19,
                    # mean 2567), so a linear penalty is driven almost entirely by
                    # a few pathological splats and conditions badly. log keeps the
                    # gradient comparable across the distribution.
                    Laniso = torch.relu(torch.log(ratio.clamp(min=1e-8) / opt.aniso_max)).mean()
